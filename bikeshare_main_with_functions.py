...
city_list = list(set((cities_lookup.keys())))
###### END OF VARIABLES FOR CHOOSING CITY ######



###### END OF VARIABLES FOR CHOOSING TIME PERIODS ######
...
month_list = list(set((month_lookup.keys()))) 
...
# Make a list of keys from the above dictionary, for purposes of detecting invalid input later.
day_list = list(set((day_lookup.keys())))
...
def filter_city():
    ...
    # The next line simply avoids having to reference the city as
...
def filter_day():
    ...
    # Make a 1st attempt to get the user's choice:
    print('\n*** Pick a day to examine, or type \"all\".')
...
'''
    ...
    # (counts unique start-end pairs; if a trip is the reverse of that mode
    # (i.e. 1st & 44th, vs. 44th and 1st), that won't be counted)
    df['combos'] = df['Start Station'] # a cheap way to make a new column
    records = df.shape[0] #number of records
    
    for i in range(records):
        df['combos'][i] = (df['Start Station'][i], df['End Station'][i])
        
    combo_mode = df['combos'].mode()[0]
    
    print("* The most common combination of stations is")
    print("   * {} (start station)".format(combo_mode[0]))
    print("   * {} (end station).".format(combo_mode[1]))


    elapsed_time = str(round((time.time() - start_time), 6))
    print("\n[This took {} seconds.]".format(elapsed_time))
    print('-'*40, '\n')
    return start_station_mode, end_station_mode
'''
